angular.module('MaxwellHealth')
    ...
    checkWidth($scope.Plans.length);
    });

    $scope.Plans = [];

    $scope.removePlan = {};
    $scope.removePlan.doClick = function(event) {
        if ($('.compare-container').length <= 3) alert("You can't have less than 2 plans at a time in the comparison screen !");
        else {
            $(event.target).closest('.compare-container').slideUp(400);
            setTimeout(function() {
                $(event.target).closest('.compare-container').remove();
            }, 600);
        }
    }
    ...
    $scope.subsidyValue = ($routeParams.plansIds).split('&')[1];

    var w = $(window);

    function checkWidth(nr) {
        var windowsize = w.width();
        nr = typeof nr !== 'undefined' ? nr+1 : $('.compare-container').length;
        console.log(nr);
        if (nr * 404 > windowsize)
            $('#main-iframe-compare').css('width', windowsize + 'px');
        else $('#main-iframe-compare').css('width', '');
    }
